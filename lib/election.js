...
module.exports = function(praetor){
    ...
    praetor.events["praetor.ballot"] = function(node){
    ...
    praetor.events["praetor.vote"] = function(vote){
        var attributes = praetor.legiond.get_attributes();

        var num_peers = praetor.legiond.get_peers().length + 1;
        votes.push(vote);

        var results = _.groupBy(votes, "node");
    ...
    praetor.events["praetor.promotion"] = function(node){
        var decision = null;
        clearTimeout(tie_election);
        praetor.legion.emit("leader_elected", node);
    }

    praetor.events["praetor.demotion"] = function(){
        praetor.actions.elect();
    }
}
