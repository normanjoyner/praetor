...
module.exports.Configure = function(praetor) {
    ...
    praetor.actions.demote = () => {
        ...
        praetor_attributes.leader = false;
    ...
    praetor.legiond.on('praetor.vote', (message) => {
        ...
        if(message.data.node === attributes.id) {
        ...
        // I have received over half the votes, promote myself as leader
            ...
            logger.log('debug', `Received ${results[attributes.id].length} out of ${(num_peers / 2)} votes (> 50%); promoting myself to controlling leader);
            praetor.actions.promote();
        }
    });
